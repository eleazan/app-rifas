...
function PrizeCard({ prize }) {
    return (
        <div className="group relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur-sm hover:bg-white/[0.08] transition-all duration-300">
            {/* Image */}
            {prize.image ? (
                <div className="aspect-[4/3] overflow-hidden bg-slate-900/60 flex items-center justify-center">
...
export default function Rifa({ raffle }) {
    ...
    const shuffledPrizes = useMemo(() => shuffle(raffle.prizes ?? []), [raffle.id]);
    const shuffledSponsors = useMemo(() => shuffle(raffle.sponsors ?? []), [raffle.id]);
                    ...
                    <div className="py-8">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
                            {shuffledSponsors.map((sponsor) => (
                                <div key={sponsor.id}
                                     className="flex flex-col items-center gap-4 p-6 rounded-2xl border border-white/10 bg-white/[0.04] hover:bg-white/[0.08] transition-colors">
                                    {sponsor.logo ? (
                                    ...
                                    <p className="text-base font-semibold text-white text-center">{sponsor.name}</p>
                                    {sponsor.website && (
