...
export default function Edit({ organizer }) {
    const { data, setData, post, processing, errors } = useForm({
        name: organizer.name,
        logo: null,
        _method: 'PUT',
    });
    const [preview, setPreview] = useState(organizer.logo);
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('organizers.update', organizer.id));
    };

    return (
        <AuthenticatedLayout header="Editar Organizador">
            <Head title="Editar Organizador" />
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 p-6 lg:p-8">
                    <form onSubmit={submit} className="space-y-5">
                        ...
                        <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                            ...
                            <Link href={route('organizers.index')} className="btn btn-ghost">
                                Cancelar
                            </Link>
                        </div>
                    </form>
